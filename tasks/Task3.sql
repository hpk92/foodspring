...
(
    ...
    from
        (
        SELECT c.Client_Name as Client_Name ,p.product as product_name, pur.amount as amount FROM foodspring.product as p, foodspring.client as c,
            (
            SELECT sum(purc.amount) as amount,purc.ClientId as ClientId, purc.ProductId as ProductId FROM foodspring.purchase as purc
